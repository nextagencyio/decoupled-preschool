...
async function getProgram(path: string): Promise<DrupalProgram | null> {
  try {
    ...
    const data = await client.raw(GET_PROGRAM_BY_PATH, { path })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getProgram(path)

  if (!item) {
    return { title: 'Program Not Found | Little Sprouts Academy' }
  }
  ...
  return (
    <div className="min-h-screen bg-amber-50">
      ...
      <section className="relative bg-gradient-to-br from-primary-600 via-primary-700 to-primary-800 text-white py-12 md:py-16 overflow-hidden">
        <div className="absolute top-8 right-[12%] w-12 h-12 bg-accent-400 rounded-full opacity-30 animate-bounce" style={{ animationDuration: '3s' }} />
        <div className="absolute bottom-8 left-[8%] w-8 h-8 bg-pink-400 rounded-lg opacity-25 rotate-12 animate-bounce" style={{ animationDuration: '4s', animationDelay: '0.5s' }} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-3xl shadow-lg shadow-primary-100/30 p-6 sticky top-24 border-2 border-primary-100">
                <h3 className="text-lg font-bold font-display text-primary-800 mb-4">Program Details</h3>
                <dl className="space-y-4">
                  {(item as any).ageRange && (
                    <div>
                      <dt className="text-sm text-gray-500">Age Range</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).ageRange}</dd>
                    </div>
                  )}
                  ...
                  {(item as any).tuition && (
                    <div>
                      <dt className="text-sm text-gray-500">Tuition</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).tuition}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
